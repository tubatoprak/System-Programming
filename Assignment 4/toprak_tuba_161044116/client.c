...
int countWords(const char* str) {
    ...
    // Stringin her karakterini kontrol et
    for (int i = 0; str[i] != '\0'; i++) {
        // Boşluk, tab veya yeni satır karakterleri bir kelimenin sonunu belirtir
        if (str[i] == ' ' || str[i] == '\t' || str[i] == '\n') {
            isWord = false;
        }
        // Diğer karakterler bir kelimenin içinde olduğunu belirtir
        else if (!isWord) {
            isWord = true;
            wordCount++;
        }
